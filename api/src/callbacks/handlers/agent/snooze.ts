// D06 — POST /api/agent/callbacks/:id/snooze — re-schedule callback.

import type { FastifyRequest, FastifyReply } from "fastify";
import { SnoozeBody } from "../../schemas.js";
import { snoozeCallback } from "../../service.js";
...
export async function handleSnoozeCallback(redis: any) {
  return async (req: FastifyRequest, reply: FastifyReply): Promise<void> => {
    ...
    try {
      const result = await snoozeCallback(getPrisma(), redis, auth, BigInt(id), parsed.data);
